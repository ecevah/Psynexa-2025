...
class MeditationIterationsController {
  ...
  async createMeditationIteration(req, res) {
    try {
      ...
      res.status(201).json({
        status: true,
        message: "Meditasyon iterasyonu oluşturuldu",
        data: meditation,
      });
    } catch (error) {
      ...
      res.status(500).json({
        status: false,
        message: "Meditasyon iterasyonu oluşturulamadı",
      });
    }
  }
  ...
  async getMeditationIterations(req, res) {
    try {
      ...
      res.json({
        status: true,
        message: "Meditasyon iterasyonları alındı",
        data: meditations,
      });
    } catch (error) {
      ...
      res
        .status(500)
        .json({ status: false, message: "Meditasyon iterasyonları alınamadı" });
    }
  }
  ...
  async getMeditationIteration(req, res) {
    try {
      ...
      if (!meditation) {
        return res
          .status(404)
          .json({ status: false, message: "Meditasyon iterasyonu bulunamadı" });
      }

      res.status(200).json({
        status: true,
        messages: "Meditasyon iterasyonu bulundu",
        data: meditation,
      });
    } catch (error) {
      ...
      res.status(500).json({
        status: false,
        message: "Meditasyon iterasyonu detayı alınamadı",
      });
    }
  }
  ...
  async updateMeditationIteration(req, res) {
    try {
      ...
      res.status(200).json({
        status: true,
        message: "Meditasyon iterasyonu güncellendi",
        data: meditation,
      });
    } catch (error) {
      ...
      res.status(500).json({
        status: false,
        message: "Meditasyon iterasyonu güncellenemedi",
      });
    }
  }
  ...
  async deleteMeditationIteration(req, res) {
    try {
      ...
      res.status(200).json({
        status: true,
        message: "Meditasyon iterasyonu başarıyla silindi",
      });
    } catch (error) {
      logger.error(`Meditasyon iterasyonu silme hatası: ${error.message}`);
      res
        .status(500)
        .json({ status: false, message: "Meditasyon iterasyonu silinemedi" });
    }
  }
}
