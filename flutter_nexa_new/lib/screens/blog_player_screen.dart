...
class BlogPlayerScreen extends StatefulWidget {
  final BlogModel blog;

  const BlogPlayerScreen({
    super.key,
    required this.blog,
  });

  @override
  State<BlogPlayerScreen> createState() => _BlogPlayerScreenState();
}

class _BlogPlayerScreenState extends State<BlogPlayerScreen>
  ...
  Future<void> _initAudioPlayers() async {
    ...
    String vocalizationUrl = widget.blog.vocalizationUrl ?? '';
    String soundUrl = widget.blog.soundUrl ?? '';

    if (vocalizationUrl.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Bu blog için sesli okuma bulunmamaktadır.'),
        ),
      );
    ...
    try {
      ...
      // Set background sound source if available
      if (soundUrl.isNotEmpty) {
        setState(() {
      ...
      // Wait for vocalization player to be ready
      await _vocalizationPlayer.processingStateStream.firstWhere(
        (state) => state == ProcessingState.ready,
        orElse: () => ProcessingState.idle,
      );

      // Also wait for background player if available
      if (soundUrl.isNotEmpty) {
        await _backgroundPlayer.processingStateStream.firstWhere(
          (state) => state == ProcessingState.ready,
          orElse: () => ProcessingState.idle,
        );
      }
      ...
      // Start playback with background sound if available
      if (soundUrl.isNotEmpty) {
  ...
  void _playPause() {
    if (_isPlaying) {
      _vocalizationPlayer.pause();
      if (widget.blog.soundUrl != null && widget.blog.soundUrl!.isNotEmpty) {
        _backgroundPlayer.pause();
      }
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    String imageUrl = '';
    if (widget.blog.backgroundUrl.isNotEmpty) {
      imageUrl = widget.blog.backgroundUrl.startsWith('/')
          ? 'https://bulunlanbunuda.psynexa.com${widget.blog.backgroundUrl}'
          : 'https://bulunlanbunuda.psynexa.com/${widget.blog.backgroundUrl}';
    }
      ...
      body: Stack(
        children: [
          ...
          SizedBox(
            ...
            child: widget.blog.backgroundUrl.isNotEmpty
                ? Image.network(
                    imageUrl,
                    fit: BoxFit.cover,
